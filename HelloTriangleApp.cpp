...
void HelloTriangleApp::createVertexBuffer() {
  ...
  if (vkCreateBuffer(device, &bufferInfo, nullptr, &vertexBuffer) != VK_SUCCESS) {
    throw std::runtime_error("FMQyD08uBP :: failed to create vertex buffer!");
  }
  ...
  if (vkAllocateMemory(device, &allocInfo, nullptr, &vertexBufferMemory) != VK_SUCCESS) {
    throw std::runtime_error("a9PZsLRjJ5 :: failed to allocate vertex buffer memory!");
  }

  // memoryOffset должно быть кратно memRequirements.alignment
